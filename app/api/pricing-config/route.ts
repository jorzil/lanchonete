...
const SYSTEM_PHONE = '__pricing_config__'

const DEFAULT_CONFIG = { config: null as unknown }
...
export async function PATCH(req: NextRequest) {
  ...
  const writeErr = await writeToDb({ config: body.config ?? null, updatedAt: new Date().toISOString() })
  if (writeErr) {
